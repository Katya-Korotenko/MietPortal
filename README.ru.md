...
> **Известное ограничение:** MySQL не поддерживает условные (частичные)
> unique-индексы (`UniqueConstraint(condition=Q(...))`). Django это
> обнаруживает и не создаёт такой constraint на уровне базы данных (см.
> предупреждение `models.W036` при `migrate`) — это касается и правила
> уникальности адреса объявления, и правила дедупликации истории
> просмотров. Оба правила всё равно соблюдаются на уровне приложения — через
> `ListingSerializer.validate()` для объявлений и через `get_or_create()`
> для истории просмотров, — поэтому бизнес-правило работает для любого
> запроса через API. Чего не хватает — это дополнительной страховки на
> уровне БД против записей в обход API (например, прямой доступ через ORM
> или Django admin). Это осознанный компромисс, продиктованный требованием
> использовать MySQL как основную базу данных.
